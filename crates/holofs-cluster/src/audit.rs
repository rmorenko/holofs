...
mod tests {
    ...
    use holofs_testutils::{spawn_mock_node, DisablePool};
    ...
    async fn audit_shard_returns_pass_for_matching_shard() {
        ...
        let h = shard_hash(&s);
        let (addr, _handle) = spawn_mock_node(Response::AuditResp { shard: Some(s) }).await;
    ...
    async fn audit_shard_returns_missing_for_none_response() {
        let _g = DisablePool::new();
        let (addr, _handle) = spawn_mock_node(Response::AuditResp { shard: None }).await;
    ...
    async fn audit_shard_returns_hash_mismatch_for_wrong_bytes() {
        ...
        let (addr, _handle) = spawn_mock_node(Response::AuditResp {
            shard: Some(actual),
        })
        .await;
    ...
    async fn audit_shard_returns_protocol_error_for_wrong_response_kind() {
        let _g = DisablePool::new();
        let (addr, _handle) = spawn_mock_node(Response::Pong).await;
        let outcome = audit_shard(&addr, 0xABCD, 0, 0, [0u8; 32]).await;
