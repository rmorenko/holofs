        ...
        <main class="container spotlight-page">
            {move || {
                ...
                if a.is_empty() {
                    ...
                    view! { <SpotlightBody a=a x=x y=y w=w h=h/> }.into_any()
                }
            }}
        </main>
    }
...
#[component]
fn SpotlightBody(a: String, x: f32, y: f32, w: f32, h: f32) -> impl IntoView {
    let enc = crate::url_encode(&a);
    let api_src = format!("/api/spotlight.png?name={enc}&x={x}&y={y}&w={w}&h={h}");
    ...
    view! {
        ...
        <PresetButtons a=a_for_form.clone()/>
...
#[component]
fn PresetButtons(a: String) -> impl IntoView {
    let a_owned = a;
    let preset = move |x: f32, y: f32, w: f32, h: f32, key: &'static str| {
        let href = format!(
            "/spotlight?a={}&x={x}&y={y}&w={w}&h={h}",
