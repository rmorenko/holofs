...
pub const TRANSLATIONS: &[(&str, [&str; 5])] = &[
    ...
    ("search.title",
        ["Semantic search",
         ...
         "Búsqueda semántica"]),
    ("search.intro",
        ["Type a natural-language description. CLIP embeds your query and ranks every catalog image against it; matches stream in as coarse thumbnails first and sharpen as the detail layers arrive.",
         "Опишите словами что ищете. CLIP эмбеддит запрос и ранжирует каждую картинку каталога. Превью появляются сначала размытыми и резкость нарастает по мере подгрузки деталей.",
         "Geben Sie eine Beschreibung in natürlicher Sprache ein. CLIP bettet die Anfrage ein und sortiert jedes Katalogbild danach; Treffer erscheinen zuerst als grobe Vorschauen und werden schärfer, sobald Detailschichten geladen sind.",
         "Décrivez ce que vous cherchez en langage naturel. CLIP encode la requête et classe chaque image du catalogue ; les résultats apparaissent d'abord en aperçus flous et se précisent à mesure que les couches de détail arrivent.",
         "Describa lo que busca en lenguaje natural. CLIP codifica la consulta y clasifica cada imagen del catálogo; los resultados aparecen primero como vistas previas borrosas y se enfocan a medida que llegan las capas de detalle."]),
    ("search.placeholder",
        ["a parrot on a green background",
         "попугай на зеленом фоне",
         "ein Papagei auf grünem Hintergrund",
         "un perroquet sur fond vert",
         "un loro sobre fondo verde"]),
    ("search.submit",
        ["Search", "Найти", "Suchen", "Rechercher", "Buscar"]),
    ("search.loading",
        ["embedding query and scanning index…",
         "эмбеддим запрос и сканируем индекс…",
         "Anfrage einbetten und Index durchsuchen…",
         "encodage de la requête et scan de l'index…",
         "codificando consulta y escaneando el índice…"]),
    ("search.results_for",
        ["results for", "результатов для", "Treffer für",
         "résultats pour", "resultados para"]),
    ("search.empty_prompt",
        ["Type a query above to search the catalog by meaning.",
         "Введите запрос выше, чтобы искать по смыслу.",
         "Geben Sie oben eine Anfrage ein, um den Katalog nach Bedeutung zu durchsuchen.",
         "Saisissez une requête ci-dessus pour fouiller le catalogue par sens.",
         "Escriba una consulta arriba para buscar por significado."]),
    ("search.no_hits",
        ["No matches above threshold. Try a broader description.",
         "Ничего не нашлось выше порога. Попробуйте более общее описание.",
    ...
    ("similar.robust_copy_blurb",
        ["Robust-copy column = low-layer overlap minus high-layer overlap. A score ≥30 flags files that preserve the target's structure (silhouette / composition) while perturbing the detail layers — the signature of a watermarked or re-encoded copy.",
         ...
         "Robust-copy-Spalte = Tiefschicht-Überlappung minus Hochschicht-Überlappung. Werte ≥30 markieren Dateien, deren Struktur (Silhouette / Komposition) erhalten ist, während die Detailschichten gestört sind — die Signatur einer Wasserzeichen- oder Re-Encoding-Kopie.",
         "Colonne robust-copy = recouvrement bas moins recouvrement haut. Un score ≥30 signale les fichiers qui préservent la structure de la cible (silhouette / composition) tout en perturbant les couches de détail — la signature d'une copie filigranée ou ré-encodée.",
         "Columna robust-copy = solape bajo menos solape alto. Un puntaje ≥30 marca archivos que preservan la estructura del objetivo (silueta / composición) mientras alteran las capas de detalle — la huella de una copia marcada o re-codificada."]),
    ("similar.robust_copy_warn",
        ["Likely a watermarked / re-encoded / lightly retouched copy: structure matches, detail differs.",
         "Похоже на копию с водяным знаком / перекодированием / лёгкой ретушью: структура совпадает, детали разные.",
         "Wahrscheinlich eine Kopie mit Wasserzeichen / Re-Encoding / leichter Retusche: Struktur passt, Detail unterscheidet sich.",
         "Probable copie filigranée / ré-encodée / légèrement retouchée : structure identique, détail différent.",
         "Probable copia con marca de agua / re-codificada / ligeramente retocada: estructura coincide, detalle difiere."]),
    ("similar.col.low_band",
        ["low-band %", "низкие слои %", "Tiefband %",
         "bande basse %", "banda baja %"]),
    ("similar.col.high_band",
        ["high-band %", "верхние слои %", "Hochband %",
         "bande haute %", "banda alta %"]),
    ("similar.col.robust_copy",
    ...
    ("versions.link_label",
        ["versions →", "версии →", "Versionen →",
         "versions →", "versiones →"]),
    ("versions.title_prefix",
        ["versions — ", "версии — ", "Versionen — ",
         "versions — ", "versiones — "]),
    ("versions.loading",
        ["loading versions…", "загружаем версии…", "Lade Versionen…",
         "chargement des versions…", "cargando versiones…"]),
    ("versions.empty",
        ["No archived versions yet. Re-upload to this name to start the history.",
         "Ещё нет архивных версий. Залейте файл повторно с тем же именем, чтобы начать историю.",
         "Noch keine archivierten Versionen. Erneut hochladen auf denselben Namen startet die Historie.",
         "Aucune version archivée. Téléversez à nouveau sous ce nom pour démarrer l'historique.",
         "Aún sin versiones archivadas. Vuelva a subir bajo el mismo nombre para iniciar el historial."]),
    ("versions.disabled",
        ["Version history is disabled on this server. Restart with --enable-versions to turn it on (prior shards stay live on the cluster across PUTs, so storage will grow).",
         "История версий выключена. Перезапустите сервер с --enable-versions (старые шарды останутся живыми на кластере при PUT-перезаписях, хранилище будет расти).",
         "Versionshistorie ist deaktiviert. Mit --enable-versions neu starten (Vorgänger-Shards bleiben bei PUT-Überschreibungen am Cluster, Speicher wächst).",
         "L'historique de versions est désactivé. Redémarrer avec --enable-versions (les shards antérieurs restent actifs sur le cluster lors des PUT, le stockage augmentera).",
         "El historial de versiones está deshabilitado. Reinicie con --enable-versions (los shards anteriores siguen activos en el cluster en PUTs, el almacenamiento crecerá)."]),
    ("versions.count_suffix",
        ["archived version(s)",
         "архивных версий",
         "archivierte Version(en)",
         "version(s) archivée(s)",
         "versión(es) archivada(s)"]),
    ("versions.blurb",
        ["Each row is a manifest archived from a prior PUT under this name. Click restore to swap the live catalog entry back to that version — the currently-live manifest gets archived first, so restore is reversible.",
         "Каждая строка — манифест предыдущего PUT под этим именем. Кнопка restore переключает активную запись каталога обратно на ту версию; текущая сперва уйдёт в архив, поэтому откат обратим.",
         "Jede Zeile ist ein bei einer früheren PUT archiviertes Manifest. „Restore“ tauscht den aktiven Katalogeintrag auf diese Version zurück — das aktuelle Manifest wird zuerst archiviert, das Restore ist also umkehrbar.",
         "Chaque ligne est un manifeste archivé lors d'un PUT antérieur. « restore » échange l'entrée active du catalogue vers cette version — l'actuel est d'abord archivé, le restore est donc réversible.",
         "Cada fila es un manifiesto archivado en un PUT anterior. «restore» intercambia la entrada activa del catálogo a esa versión — el actual se archiva primero, por lo que restore es reversible."]),
    ("versions.col.when",
        ["archived (UTC)", "архивировано (UTC)", "archiviert (UTC)",
         "archivé (UTC)", "archivado (UTC)"]),
    ("versions.col.kind",
        ["kind", "тип", "Art", "type", "tipo"]),
    ("versions.col.size",
        ["dimensions", "размеры", "Maße", "dimensions", "dimensiones"]),
    ("versions.col.cid",
        ["data_cid", "data_cid", "data_cid", "data_cid", "data_cid"]),
    ("versions.col.actions",
        ["actions", "действия", "Aktionen", "actions", "acciones"]),
    ("versions.action.restore",
        ["restore", "восстановить", "wiederherstellen",
         "restaurer", "restaurar"]),
    ("versions.restore_confirm",
        ["Restore this version? The currently-live manifest will be archived first.",
         "Восстановить эту версию? Текущая сперва будет архивирована.",
    ...
    ("spotlight.link_label",
        ["spotlight →", "spotlight →", "spotlight →",
         "spotlight →", "spotlight →"]),
    ("spotlight.title_prefix",
        ["spotlight — ", "spotlight — ", "Spotlight — ",
         "spotlight — ", "spotlight — "]),
    ("spotlight.missing_a",
        ["spotlight: missing ?a=<name>",
         "spotlight: не задан ?a=<имя>",
         "spotlight: ?a=<name> fehlt",
         "spotlight : ?a=<name> manquant",
         "spotlight: falta ?a=<nombre>"]),
    ("spotlight.intro",
        ["Two decodes — coarse L0 everywhere, full resolution inside the rectangle — composited per-pixel by the gateway. Picks one shard set for each region of the image instead of one for the whole frame. Bandwidth is the same as a full fetch; the demo is the visual: where you focus, the image clears up.",
         "Два прохода декода — L0 везде, полное разрешение внутри прямоугольника — пиксельный композит на сервере. Один и тот же файл, два разных уровня детализации, выбранных пространственно. Трафик такой же как у полного запроса; смысл — визуальный: где направил внимание, там и резкость.",
         "Zwei Decode-Durchgänge — grobes L0 überall, volle Auflösung im Rechteck — pro Pixel auf dem Gateway zusammengesetzt. Eine Datei, zwei Detailstufen, räumlich gewählt. Bandbreite wie beim Vollabruf; der Effekt ist visuell: wo der Fokus liegt, dort wird scharf.",
         "Deux décodes — L0 partout, pleine résolution dans le rectangle — composé pixel par pixel sur le gateway. Même fichier, deux niveaux de détail choisis spatialement. Le trafic reste celui d'un fetch complet ; l'effet est visuel : là où vous regardez, l'image se précise.",
         "Dos pasadas de decodificación — L0 en todo, resolución completa dentro del rectángulo — compuestas pixel a pixel en el gateway. Mismo archivo, dos niveles de detalle elegidos espacialmente. El tráfico es el de un fetch completo; el efecto es visual: donde mira, la imagen se enfoca."]),
    ("spotlight.custom_summary",
        ["custom ROI (0..1 floats)",
         "произвольный ROI (0..1)",
         "eigener ROI (0..1)",
         "ROI personnalisé (0..1)",
         "ROI personalizado (0..1)"]),
    ("spotlight.apply",
        ["apply", "применить", "anwenden", "appliquer", "aplicar"]),
    ("spotlight.field.x",
        ["x", "x", "x", "x", "x"]),
    ...
    ("spotlight.field.h",
        ["h", "в", "H", "h", "al"]),
    ("spotlight.preset.center",
        ["center 30%", "центр 30%", "Zentrum 30%",
         "centre 30%", "centro 30%"]),
    ("spotlight.preset.tl",
        ["top-left ¼", "верх-лев ¼", "oben-links ¼",
         "haut-gauche ¼", "arriba-izq ¼"]),
    ("spotlight.preset.tr",
        ["top-right ¼", "верх-прав ¼", "oben-rechts ¼",
         "haut-droite ¼", "arriba-der ¼"]),
    ("spotlight.preset.bl",
        ["bottom-left ¼", "низ-лев ¼", "unten-links ¼",
         "bas-gauche ¼", "abajo-izq ¼"]),
    ("spotlight.preset.br",
        ["bottom-right ¼", "низ-прав ¼", "unten-rechts ¼",
         "bas-droite ¼", "abajo-der ¼"]),
    ("spotlight.preset.hbar",
        ["horizontal strip", "горизонтальная полоса", "horizontaler Streifen",
         "bande horizontale", "franja horizontal"]),
    ("spotlight.preset.vbar",
        ["vertical strip", "вертикальная полоса", "vertikaler Streifen",
         "bande verticale", "franja vertical"]),
    ("spotlight.mode_label",
        ["render mode:", "режим рендера:", "Rendermodus:",
         "mode de rendu :", "modo de render:"]),
    ("spotlight.mode.spatial",
        ["spatial composite", "пространственная композиция", "räumliche Komposition",
         "composition spatiale", "composición espacial"]),
    ("spotlight.mode.coeff",
        ["coefficient mask (Haar)", "маска коэффициентов (Haar)", "Koeffizientenmaske (Haar)",
         "masque de coefficients (Haar)", "máscara de coeficientes (Haar)"]),
    ("spotlight.caption_prefix",
        ["ROI (x, y, w, h) =",
         "ROI (x, y, ш, в) =",
         "ROI (x, y, B, H) =",
         "ROI (x, y, l, h) =",
    ...
    ("holo.title_prefix",
        ["streaming hologram —",
         "потоковая голограмма —",
         "Streaming-Hologramm —",
         "hologramme streaming —",
         "holograma streaming —"]),
    ("holo.missing_name",
        ["holo: missing /holo/<name>",
         "holo: не задан /holo/<name>",
         "holo: kein /holo/<name>",
         "holo : /holo/<name> manquant",
         "holo: falta /holo/<name>"]),
    ("holo.intro",
        ["The image below is being delivered as a multipart stream of progressively-decoded PNG frames. The first frame is just L0 (≈8% of the bytes); each subsequent frame adds the next DWT layer. The browser swaps the rendered pixels as parts arrive — no JavaScript involved. Best viewed on a fresh request (a cached run is too fast to see the steps).",
         "Картинка ниже приходит по сети как поток multipart-частей с постепенно декодированными PNG. Первая часть — только L0 (~8% объёма), каждая следующая добавляет очередной слой DWT. Браузер сам подменяет содержимое <img> по мере прихода частей — без JavaScript. Эффект лучше всего видно при первом запросе (повторный кэшируется и проходит мгновенно).",
         "Das Bild unten wird als multipart-Strom progressiv dekodierter PNG-Frames geliefert. Der erste Frame ist nur L0 (~8 % der Bytes); jeder weitere fügt eine DWT-Schicht hinzu. Der Browser tauscht den Inhalt aus, sobald die Teile eintreffen — kein JavaScript. Effekt am besten beim Erstaufruf sichtbar (gecachte Läufe sind zu schnell).",
         "L'image ci-dessous est diffusée comme un flux multipart de PNG décodés progressivement. Le premier frame n'est que L0 (~8 % des octets) ; chaque suivant ajoute la couche DWT suivante. Le navigateur remplace le rendu à mesure que les parties arrivent — sans JavaScript. Effet plus visible à la première requête.",
         "La imagen abajo se entrega como un flujo multipart de PNGs decodificados progresivamente. El primer frame es solo L0 (~8 % de los bytes); cada siguiente añade la próxima capa DWT. El navegador reemplaza el contenido al llegar las partes — sin JavaScript. Efecto más visible en el primer pedido."]),
    ("holo.how_h",
        ["What's happening on the wire",
         ...
         "Qué pasa en la red"]),
    ("holo.step1",
        ["Server fetches the L0 shards from the cluster, decodes them, encodes a tiny PNG, writes it into the response. First frame is on the wire in tens of milliseconds.",
         "Сервер тянет с кластера L0-шарды, декодирует, пакует крошечный PNG и пишет его в ответ. Первый кадр уходит в сеть за десятки миллисекунд.",
         "Server holt L0-Shards aus dem Cluster, dekodiert, packt ein winziges PNG und schreibt es in die Antwort. Erster Frame ist in zehn Millisekunden auf der Leitung.",
         "Le serveur récupère les shards L0 du cluster, les décode, encode un petit PNG et l'écrit dans la réponse. Premier frame en quelques dizaines de ms.",
         "El servidor obtiene los shards L0 del cluster, los decodifica, codifica un PNG pequeño y lo escribe en la respuesta. Primer frame en decenas de ms."]),
    ("holo.step2",
        ["Without closing the response, the server fetches L1 shards, decodes them on top of the L0 plane, encodes a sharper PNG, writes it as the next multipart part. Repeats for L2, L3, … up to full resolution.",
         "Не закрывая ответ, сервер берет L1-шарды, декодирует их поверх L0-плоскости, кодирует более резкий PNG и шлёт следующей multipart-частью. Дальше L2, L3, … до полной чёткости.",
         "Ohne die Antwort zu schließen, holt der Server L1-Shards, dekodiert sie auf der L0-Ebene, kodiert ein schärferes PNG und sendet es als nächste multipart-Teil. So weiter bis zur Vollauflösung.",
         "Sans fermer la réponse, le serveur prend les shards L1, les décode sur le plan L0, encode un PNG plus net et l'envoie comme partie suivante. Ainsi de suite jusqu'à la pleine résolution.",
         "Sin cerrar la respuesta, el servidor toma los shards L1, los decodifica sobre el plano L0, codifica un PNG más nítido y lo envía como siguiente parte. Así sucesivamente."]),
    ("holo.step3",
        ["The browser treats `multipart/x-mixed-replace` as a flip-book: it renders each part as a new `<img>` body, replacing the previous one. With holofs's layered shards, that means a thumbnail's worth of bytes gets you the first visible frame and every later frame adds detail.",
         "Браузер понимает `multipart/x-mixed-replace` как флипбук: каждая часть подменяет содержимое `<img>` предыдущей. С нашими слоистыми шардами это значит, что первый видимый кадр требует трафика как у миниатюры, а каждый следующий добавляет деталей.",
         "Der Browser interpretiert `multipart/x-mixed-replace` als Daumenkino: jede Teil-Antwort ersetzt den `<img>`-Inhalt. Mit unseren Schicht-Shards heißt das: erster sichtbarer Frame kostet so viel Traffic wie ein Thumbnail, jeder weitere fügt Detail hinzu.",
         "Le navigateur traite `multipart/x-mixed-replace` comme un flip-book : chaque partie remplace le contenu de la précédente. Avec nos shards par couche, le premier frame visible coûte autant qu'une vignette ; chaque suivant ajoute du détail.",
         "El navegador trata `multipart/x-mixed-replace` como un flip-book: cada parte reemplaza el contenido de la anterior. Con nuestros shards por capa, el primer frame visible cuesta como una miniatura; cada siguiente añade detalle."]),
    ("holo.see_also",
        ["See also:", "См. также:", "Siehe auch:",
         "Voir aussi :", "Vea también:"]),
    ("holo.see_shards",
        ["shards inspector", "инспектор шардов", "Shard-Inspektor",
         "inspecteur de shards", "inspector de shards"]),
    ("holo.see_about",
        ["why holofs", "зачем holofs", "warum holofs",
         "pourquoi holofs", "por qué holofs"]),
    ...
    ("search.disabled",
        ["Semantic search is disabled on this server. Restart with --enable-embed to turn it on (the first query downloads ~155 MiB of CLIP weights).",
         ...
         "Semantische Suche ist auf diesem Server deaktiviert. Mit --enable-embed neu starten (die erste Anfrage lädt ~155 MiB CLIP-Gewichte).",
         "La recherche sémantique est désactivée sur ce serveur. Redémarrer avec --enable-embed (la première requête télécharge ~155 Mio de poids CLIP).",
         "La búsqueda semántica está deshabilitada. Reinicie con --enable-embed (la primera consulta descarga ~155 MiB de pesos CLIP)."]),
];
