...
1. [Поле Галуа GF(2⁸)](#1-поле-галуа-gf28)
...
так что слой 0 (LL) хранится с $\lceil K \cdot 4.0 \rceil = 64$
шардами, а слой 3 (самая тонкая детализация) получает
...
в байты). Мы вычисляем 16-байтовый fingerprint как

$$
\mathrm{fp}_i \;=\; \mathrm{mean}(\mathrm{payload}(\mathrm{shard}_i)), \quad i = 0, 1, \ldots, K - 1.
$$

Для нашего $K = 16$ это даёт $4 \times 4$-сетку средних яркостей —
...
Подобие в % — $100 \cdot (1 - d / 4080)$. Идентичное содержимое → 0.
Визуально похожее → $d \lesssim 200$. Случайные изображения →
$d \gtrsim 1500$.

**Важно**: мы вычисляем этот fingerprint **без декомпрессии объекта** —
