...
**случайные** векторы коэффициентов. Это **пороговый erasure-код, не
Шамир**: вектор коэффициентов для каждой доли лежит внутри самого
файла `.holoshare` (поле `coeffs`), так что любые $K - 1$ долей дают
атакующему $K - 1$ линейных уравнений над $\mathrm{GF}(2^8)$ с $K$
неизвестными — решение сжимается до одномерного подпространства
вместо всего $\mathrm{GF}(2^8)^K$. Это примерно $(K-1)/K$ информации
открытого текста; оставшиеся $1/K$ — единственный настоящий секрет.
Для открытого текста с самопроверяющейся структурой (seed-фразы,
словарные слова, ID с чек-суммами) атакующий тривиально перебирает
последнее измерение и восстанавливает весь секрет.

**Следствие:** эта конструкция — хорошая обфускация и хороший
availability-примитив (любые $K$ долей реконструируют байт-в-байт),
но она **не** информационно-теоретически безопасна и **не** Шамир.
Не считайте `.holoshare` файлы безопасными для передачи недоверенным
сторонам. Оберните открытый текст в аутентифицированный шифр
(паттерн Кравчика «encrypted-then-shared», см. ниже), если нужна
настоящая секретность.

### holofs-escrow

`holofs-analytics::escrow` строится на
`holofs-core::rlnc::encode_layer_with_k_random` с выбираемыми
пользователем $K, N$. Каждая из $N$ долей — свежая случайная линейная
комбинация $K$ чанков открытого текста; систематический путь общего
RLNC-энкодера (который выдал бы $\min(K, N)$ долей, копирующих чанк
открытого текста как есть) в escrow намеренно пропущен, чтобы ни одна
доля не была буквальной копией плейнтекста. Свойство
threshold-erasure выше по-прежнему применимо: $K$ долей восстанавливают
точно, $K-1$ утекают ≈$(K-1)/K$ открытого текста. Доли сериализуются
в файлы `.holoshare`, распространяемые людям / устройствам.
Escrow-workflow *чисто клиентский*: на кластере ничего не хранится.
