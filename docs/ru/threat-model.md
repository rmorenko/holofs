...
| I6 | Утечка holoshare                                       | Отдельный файл `.holoshare` — одна доля из `(k,n)` Shamir-через-RLNC разделения. Обладание меньшим чем `k` числом безопасно с теоретико-информационной точки зрения (см. [theory.md §8](./theory.md#8-схема-разделения-секрета-шамира--rlnc)). |
