...
| Gateway ↔ Node     | Ed25519 challenge-response (+ опциональный mTLS) | Plain TCP или rustls TLS через `--tls` | Wire-protocol nonce + подписанный handshake; `--mtls` добавляет проверку X.509-сертификата |
...
| I2 | Противник собирает ≥ K shard'ов одного объекта      | RLNC над публичным GF(2⁸) **не** является схемой шифрования. Любые K линейно независимых shard'ов реконструируют payload. Меры защиты: **шифрование at-rest на каждой node**  и **разнообразие размещения** — при `RendezvousZoneAware` K shard'ов охватывают ≥ K разных node в ≥ ⌈K/zone_count⌉ зонах, поэтому их чтение требует компрометации стольких же. |
...
| **D**isclosure     | См. §4.4 — I1–I6.                        | At-rest шифрование |
...
4. **Криптографический anti-replay на wire frames.** Только
   `AuthChallenge` несёт nonce. добавляет per-session keying.
5. **Квантовая стойкость.** Ed25519 и SHA-256 — pre-quantum.    оценивает миграцию на PQ.
...
|-----------------------------------------------------|:-----------:|:-----------:|-------------------------|
| Wire-трафик в открытом виде в общем LAN             | Низкая      | Низкая      | Смягчается `--tls` (rustls TLS 1.2/1.3). Операторы, не устанавливающие `--tls`, должны ограничиться приватным VLAN. |
