...
- Сетевой протокол между ними (см. [api.md §2](./api.md#2-сетевой-протокол-tcp)).
...
| # | Угроза                                                  | Мера защиты |
|---|---------------------------------------------------------|-------------|
| S1 | Атакующий выдаёт себя за ноду, чтобы получать шарды    | Ed25519 challenge (`AuthChallenge`) — шлюз проверяет подпись pubkey'ем из whitelist до того, как доверяет любому ответу. См. [api.md §2 handshake](./api.md#handshake-аутентификации). |
...
| I6 | Утечка holoshare                                       | Отдельный файл `.holoshare` — одна доля из `(k,n)` **порогового RLNC-erasure разделения — НЕ Шамир, НЕ ITS-безопасно**. Обладание `k-1` долями утекает ≈(k-1)/k открытого текста (см. [theory.md §8](./theory.md#8-схема-разделения-секрета-шамира--rlnc)) — открытые тексты с самопроверяющейся структурой (seed-фразы, ключи, ID) добираются перебором последнего измерения. Меры защиты: считайте `.holoshare` только availability-бэкапом; **encrypt-then-share** (оберните открытый текст в аутентифицированный шифр, распределяйте доли шифротекста), если нужна настоящая секретность. |
...
| D3 | Скоординированное падение половины кластера            | Margin рассчитан на **одну любую зону + разрозненные одиночные отказы** (см. [theory.md §4](./theory.md#4-приоритетные-слои-и-голографическая-деградация)). Более крупные простои деградируют плавно: L3 (косметическая детализация) теряется первым, затем L2, L1. |
